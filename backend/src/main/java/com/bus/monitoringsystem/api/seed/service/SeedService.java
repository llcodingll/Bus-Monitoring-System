...
public class SeedService {
    ...
    private void insertRouteStops(Map<String, Route> routes, Map<String, Stop> s) {
        ...
        routeStopRepository.saveAll(List.of(
                // 370 OUTBOUND: 상계동→노원역→미아사거리→성신여대입구→한성대입구→혜화역→서울역
                ...
                // 471 OUTBOUND: 중계동→광운대역→석계역→혜화역→을지로3가→서울역
                ...
                // 261 OUTBOUND: 신촌역→홍대입구역→합정역→성수역→건대입구역→잠실역
                ...
                // 143 OUTBOUND: 도봉산역→방학역→창동역→노원역→길음역→서울역
